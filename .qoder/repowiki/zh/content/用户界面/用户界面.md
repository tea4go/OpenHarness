...
Hook->>Proc : 子进程启动与stdin写入
Proc-->>Hook : stdout逐行输出(带OHJSON : 前缀)
Hook->>Hook : 解析事件/增量缓冲/快照对比
...
## 结论
该TUI系统以Ink为基础，结合自研事件协议与增量缓冲策略，在TTY环境中实现了高实时性与良好交互体验。组件职责清晰、主题可插拔、命令与模态体系完善，适合在复杂AI工作流场景中作为核心交互入口。
